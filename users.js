      ...
      users
        ...
        .then((response) => {
          res.json({
            status: 200,
            data: response,
            msg: "Welcome to Whispering shouts",
          });
        })
        .catch((err) => {
          res.send(err);
...
//save a singe user to database create
...
router.route("https://xyz.com/build/getaddress/:id").get(function (req, res) {
  const id = req.params.id;
  Users.findById(id, function (err, user) {
    if (user) {
...
//delete a card
router.route("/add_review/:user").post(function (req, res) {
